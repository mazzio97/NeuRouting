...
class GreedyRepair(LNSOperator):
    """Repair solutions by picking the nearest customers."""
    ...
    def __call__(self, solutions: Sequence[VRPSolution]
        ...
        for solution in solutions:
            ...
            while len(solution.missing_customers()) > 0:
                ...
                mask = np.array([False] * (instance.n_customers + 1),
                                dtype=bool)
                mask[missing] = True
                cust = self.rng.choice(missing)
                ...
                for neigh in nearest:
                    ...
                    if (cust_route.total_demand() + neigh_route.total_demand()
                            <= instance.capacity):
                        cust_route.append_route(neigh_route, self_head,
                                                neigh_head)
